...
function Skills() {
  ...
  return (
    <>
      {isMobile && <Heading heading="Skills" />}
      <section className="section section3" id="skillsSection" name="skills">
        <div className="site-container">
          <InView onChange={(inView) => inView && scrollChange("skills")}>
            <Row around="xs" middle="xs" center="xs" start="lg">
              ...
              <Col xs={12} lg={6}>
                <Fade direction="up" cascade>
                  <Row around="xs" middle="xs" className="skills_section_skillsSets">
                    {skillsSection.skillsSet.map((skill, i) => (
                      <Col xs={2} key={i}>
                        <i className={`${skill.fontAwesomeClassname} skill_img`}></i>
                      </Col>
                    ))}
                  </Row>
                  {skillsSection.skillsTagLines.map((tagline, i) => (
